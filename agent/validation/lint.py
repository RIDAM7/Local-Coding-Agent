...
class LintValidator:
    def __init__(self, executor: Executor):
        ...
        self.command = settings.lint_command
    ...
    async def validate(self) -> ValidationDiagnostic:
        if not self.command:
            return ValidationDiagnostic(
                ...
                command="",
                success=True,
                stdout="Skipped: No lint command configured.",
                stderr="",
                exit_code=0,
                duration=0.0
            )
            
        logger.info(f"Running lint validation: {self.command}")
        ...
        if not success:
            logger.warning(f"Lint validation failed with exit code {result.exit_code}")
